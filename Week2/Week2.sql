...
--------------------------------------------------------
SELECT site_id,
	   CAST(TO_CHAR(date, 'YYYY') AS NUMERIC) AS year,  
	   CAST(TO_CHAR(date, 'MM') AS NUMERIC)  AS month, 
	   CAST(TO_CHAR(date, 'DD') AS NUMERIC)  AS day,
...
SELECT site_name, regexp_match(site_name, '^C\w+')
FROM epa_site_location
WHERE regexp_match(site_name, '^C\w+') IS NOT NULL

SELECT site_name, regexp_match(site_name, 'Street$')
FROM epa_site_location
WHERE regexp_match(site_name, 'Street$') IS NOT NULL

SELECT site_name, regexp_match(site_name, '[A-Z,0-9]$')
FROM epa_site_location
WHERE regexp_match(site_name, '[A-Z,0-9]$') IS NOT NULL

SELECT site_name, regexp_match(site_name, '^\w+\s\w+\s\d+$')
FROM epa_site_location
WHERE regexp_match(site_name, '^\w+\s\w+\s\d+$') IS NOT NULL

SELECT site_name, regexp_match(site_name, '^\w+\s\w+\s*\d*$')
FROM epa_site_location
WHERE regexp_match(site_name, '^\w+\s\w+\s*\d*$') IS NOT NULL
	
SELECT site_name, regexp_match(site_name, '^San\s\w{1,4}\s')
FROM epa_site_location
WHERE regexp_match(site_name, '^San\s\w{1,4}\s') IS NOT NULL
--------------------------------------------------------
-- Ex 4.
--------------------------------------------------------
