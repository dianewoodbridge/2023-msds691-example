...
-------------------------------------------------------
---- Ex 9. Create a CTE called cte_aqi_2020_01 
----  which only stores January of 2020 data from epa_air_quality.
----  Using the CTE, return the first 10 rows ordered by date and site_id
-------------------------------------------------------
WITH cte_aqi_2020_01 AS 
(SELECT *
FROM epa_air_quality
WHERE EXTRACT(YEAR FROM date) = 2020 AND EXTRACT(MONTH FROM date) = 1)
SELECT * FROM cte_aqi_2020_01 ORDER BY date, site_id LIMIT 10;
